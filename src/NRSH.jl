@inline function finalize_costs_nr_shortest(u, ie, patg, l, c, σ, C, Σ)
...
function nrsh(fn, sep, verbose_step)
    ...
    cstar::Array{Int64} = [typemax(Int64) for _ = 1:n]
    ...
    for s in 1:n
        ...
        for ei in 1:M
            ...
            if (ie >= l[u])
                finalize_costs_nr_shortest(u, ie, patg, l, c, σ, C, Σ)
            end
            if (u == s)
                C[ei] = 1
                Σ[ei] = 1
            ...
            if (C[ei] != typemax(Int64))
                if (C[ei] <= c[v])
                    ...
                    finalize_costs_nr_shortest(v, a - 1, patg, l, c, σ, C, Σ)
                    if (C[ei] < c[v])
                        c[v] = C[ei]
                        σ[v] = 0
                    end
